...
return null;
    ...
    public void updateJob(JobPost jp) {
       for(JobPost jobp : jobs){
           if(jobp.getPostId() == jp.getPostId()){
               jobp.setPostProfile(jp.getPostProfile());
               jobp.setPostDesc(jp.getPostDesc());
               jobp.setPostTechStack(jp.getPostTechStack());
               jobp.setReqExperience(jp.getReqExperience());
           }
        }
